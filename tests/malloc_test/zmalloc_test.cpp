...
s32 zmalloc_stress()
{
    ...
    PROF_OUTPUT_MULTI_COUNT_CPU("st_free(st_malloc(0~1024))", rand_size, cost.stop_and_save().cost());
    ...
    for (size_t loop = 0; loop < 40; loop++)
    {
        ...
        PROF_OUTPUT_MULTI_COUNT_CPU("st_malloc(0~512k)", buffers->size(), cost.stop_and_save().cost());
        PROF_START_COUNTER(cost);
        for (auto p : *buffers)
        {
            st_free(p);
        }
        PROF_OUTPUT_MULTI_COUNT_CPU("st_free(0~512k)", buffers->size(), cost.stop_and_save().cost());
        buffers->clear();
    }
    ...
    */
    ...
    for (size_t loop = 0; loop < 80; loop++)
    {
        ...
        PROF_OUTPUT_MULTI_COUNT_CPU("st_free bat", buffers->size(), cost.stop_and_save().cost());
        buffers->clear();
    }
    PROF_OUTPUT_SELF_MEM("st malloc finish");
...
s32 zmalloc_base_test()
{
    ...
    PROF_OUTPUT_MULTI_COUNT_CPU("zmalloc third used", 1000 * 10000 - 1024, cost.stop_and_save().cost());
