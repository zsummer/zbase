...
public:
	...
	std::pair<bool, zpoint_hit> capsule_capsule(const zpoint_capsule& scapsule, const zpoint_capsule& tcapsule)
	{
		...
		{
			float projection_len = u.dot(tcapsule.a_ - scapsule.a_) / u.normalize();
			zpoint virtual_pos = scapsule.a_ + u.normalize() * projection_len;

			//
			if ((virtual_pos - tcapsule.a_).normalize() <= (std::max)(zpoint::kFloatPrecision, scapsule.r_ + tcapsule.r_))
			{
				//collinear
				// 
				// projection_len = u.dot(tcapsule.a_ - scapsule.a_) / u.normalize();
				...
				projection_len = u.dot(tcapsule.b_ - scapsule.a_) / u.normalize();
				virtual_pos = scapsule.a_ + u.normalize() * projection_len;
				if (projection_len >= 0 && projection_len <= u.length())
				{
					result.first = true;
				...
				projection_len = v.dot(scapsule.a_ - tcapsule.a_) / v.normalize();
				...
				projection_len = v.dot(scapsule.b_ - tcapsule.a_) / v.normalize();
				virtual_pos = tcapsule.a_ + v.normalize() * projection_len;
				if (projection_len >= 0 && projection_len <= v.length())
				{
					result.first = true;
		...
		{
			result.first = true;
			result.second.hit_dir_ = v.const_normalize();
			result.second.hit_pos_ = tcapsule.a_ + result.second.hit_dir_;
			return result;
		}
		...
		float projection_len = u.dot(v) / u.distance();
		float u_len = u.length();

		projection_len = (std::min)(projection_len, u_len);


		result = sphere_sphere_pos(spos, sr, u.const_normalize() * projection_len, tcapsule.r_);
		return result;
	}
